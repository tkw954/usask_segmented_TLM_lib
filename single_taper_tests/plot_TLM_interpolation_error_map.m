...
lambda_mat=(tmp.lambda(1:(end-1))+tmp.lambda(2:end))/2;
beta_mat=10.^((log10(tmp.beta(1:(end-1)))+log10(tmp.beta(2:end)))/2);
...
for i=1:N_lambda
    ...
    for j=1:N_beta
        ...
        if isempty(which('MOCinit'))
            addpath('D:\work\usask_research\projects\transmission_line\analysis\library_functions\MOC_solver');
        end
        
        if isempty(which('QA_unit_MOC'))
            addpath('D:\work\usask_research\projects\transmission_line\analysis\tapered_FIR\curve_fit_inlet\anechoic_inlet_H');
        end
